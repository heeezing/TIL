...
public class MemberDAO {
	...
	public void insertMember(MemberVO member) throws Exception{
		...
		try {
			...
			//오토커밋 해제
			//(sql문 1개 : 오토커밋 / 2개 이상 : 오토커밋 해제, 수작업 필요) 
	...
	public void updateMember(MemberVO member) throws Exception{
		...
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "UPDATE zmember_detail "
				+ "SET name=?,phone=?,email=?,zipcode=?,address1=?,address2=?,modify_date=SYSDATE "
				+ "WHERE mem_num=?";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			//?에 데이터 바인딩
	...
	public int getMemberCountByAdmin(String keyfield, String keyword) throws Exception{
		...
		try {
			...
			//SQL문 작성
			sql = "SELECT COUNT(*) FROM zmember m LEFT OUTER JOIN zmember_detail d "
		...
		try {
			...
			sql = "SELECT * "
				+ "FROM (SELECT a.*, rownum rnum "
				       + "FROM (SELECT * "
				       		 + "FROM zmember m LEFT OUTER JOIN zmember_detail d USING(mem_num) " 
				       		 + sub_sql 
